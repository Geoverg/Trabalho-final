...
function resetView() {
  ...
  highlightBusca = null;
  highlightNos = [];
  escreverSaida("reset", "Visualização redefinida (Você voltou ao topo).");
  desenhar();
}
...
function desenhar() {
  ...
  if (arvore.raiz) {
    desenharNo(arvore.raiz, canvas.width / 2 / escala - deslocX / escala, 40, canvas.width / 4 / escala);
  }
  ctx.restore();
}
...
function desenharNo(no, x, y, espaco) {
  ...
  if (no.esquerda) {
    ...
    ctx.stroke();
    desenharNo(no.esquerda, x - espaco, y + 60, espaco / 1.5);
  }
  if (no.direita) {
    ctx.beginPath();
    ctx.moveTo(x, y);
